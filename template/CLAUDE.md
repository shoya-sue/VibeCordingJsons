...
- **Git identity guard** (`guard-git-identity.sh` PreToolUse + `git-identity-sentinel.sh` SessionStart, wired in `settings.json`): Claude は `git config --global/--system user.name/user.email` を実行できない（deny + hook で二重ブロック）。`Test User`/`test@test.com` 等のテスト identity 設定も遮断。per-repo は `--local`、マシン全体設定はユーザーが手動で（`!` bang はガードを通らない）。SessionStart センチネルはグローバル identity が既にテスト値なら警告する。**背景**: 2026-07-01、隔離不足のテストハーネス（サードパーティ/CI のデフォルト ID）が `~/.gitconfig` を `Test User <test@test.com>` に上書きし、お客さん案件を含めコミット著者が誤記録される事故が発生 → その恒久ガード
...
- v2.1.198+ の主な変更: **Claude in Chrome が GA**、**Explore agent がデフォルトでメインセッションのモデルを継承**（**≤opus**、以前は haiku 固定）、**subagent / context compaction が session の extended thinking を継承**、`claude agents` 起動の **background agent が worktree でコード作業を終えると自動で commit/push/draft-PR**（確認なし）、bundled **`/dataviz` skill 追加**（chart/dashboard 設計 + palette validator）、`Notification` hook が background agent の待機/完了で発火（`agent_needs_input` / `agent_completed`）、mid-response の transient エラー（ECONNRESET 等）が backoff で自動 retry
- Voice input（日本語）: ネイティブ `/voice` を `settings.json` で有効化（`voice.enabled: true` / `mode: tap`）＋ `language: japanese` で日本語ディクテーション最適化。`language` は応答言語も日本語化する。無効化は `/voice off` または `voice.enabled: false`。整形/オフライン経路は `voice-input` スキル（`.claude/skills/voice-input/`、ローカル whisper.cpp スクリプト同梱）。macOS は初回マイク許可が必要。v2.1.176+ では session title も `language` の言語で自動生成される（`language: japanese` なら日本語タイトル）。v2.1.195+ で日本語など**スペース無し言語の音声 auto-submit** が正しく発火するよう修正された
