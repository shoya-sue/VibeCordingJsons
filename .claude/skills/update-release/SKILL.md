---
name: update-release
description: VibeCordingJsons 専用のリリース運用 skill。Claude Code / ECC の最新変更を調査し、更新が必要かを判定 → 必要なら実装 → feature branch で commit/PR/merge → install.sh で検証 → GitHub release を発行 → follow-up issue を整理し、context・memory・Obsidian の 3 面へ反映する。末尾に go を付けると全ゲートを承認済みとして完走する。プロジェクトローカル（このリポジトリでのみ利用可、グローバル/テンプレ配布しない）。
...
`develop → git commit/push/PR → merge → git release → git issue → obsidian update` の一気通貫を 1 実行で完走させる。各フェーズと本文ステップの対応:

| フェーズ | ステップ |
|---------|---------|
| develop | Pre-flight / Step 0・0.5・1・2 |
| git commit push pr | Step 4 |
| merge | Step 5 |
| git release | Step 6（install 検証）・Step 7（release 発行） |
| git issue | Step 8（follow-up 作成 + 条件充足 issue の close） |
| obsidian update | Step 9（memory）・Step 10（obsidian project note） |

どのフェーズが 1 つでも欠けたら未完了。最後に Step 11 のレポートで全フェーズの結果を報告する。

### 実行モード（`go`）

引数末尾または同一ターンのユーザー発話に `go`（同義: `merge` / `proceed` / 「完走」）が含まれる場合、**完走モード**として全 BLOCKING ゲート（Step 5 merge / Step 7 release / Step 8 issue）を承認済みとみなし、停止せず最後まで走る。ただし以下は完走モードでも必ず停止する:

- Step 6 の validation gate が `PATH_NOT_FOUND ✗`（壊れた設定を release しない）
- `gh pr merge` がブランチ保護で失敗（UI merge を待つ）
- その他、破壊的・非可逆でユーザー承認が別途必要な操作

`go` が無い場合は Step 5 の手前で PR URL を提示して確認待ちする（従来挙動）。
...
> | **memory** | `~/.claude/projects/-Users-shoya-sue-Public-shoya-sue-VibeCordingJsons/memory/project_vibecording.md`（このディレクトリは **symlink → Obsidian `90_artifacts/claude-code/memory/VibeCordingJsons/`** なので vault に自動ミラーされる） | Step 9 |
> | **obsidian** | project note `20_projects/shoya-sue/VibeCordingJsons.md`（手動運用ノート。memory ミラーとは別物） | Step 10 |
...
# Open follow-up issues（前回までのリリースで「後で対応」とした項目）
...
- `OPEN_FOLLOWUPS` — 現在 open な follow-up issue（番号 + close 条件）。Step 8 で棚卸しする
...
### 8. GitHub issue（= git issue 面の更新）

リリースで生じた follow-up を GitHub issue として管理する。完走モードでは自動で実行するが、close/create の前に対象と理由を 1 行ずつ提示する。2 部構成:

#### 8-a. 条件充足した既存 follow-up issue を close

Pre-flight で記録した `OPEN_FOLLOWUPS` を 1 件ずつ確認し、今回のリリースで close 条件を満たしたものを閉じる:

```bash
gh issue view <N>            # close 条件を再確認
gh issue close <N> --comment "<充足した条件と対応リリース vX.Y.Z を明記>"
```

- 日付条件（例「8/31 後に見直す」）は今日の日付で判定。未達なら open のまま残す。
- docs 公開待ち等の条件は該当ソースを `WebFetch` / `gh` で確認してから close。
- 今回のリリース内で実装として取り込んだ follow-up も close 対象。

#### 8-b. 新しい follow-up を issue 化

今回のトリアージで ⚪ スキップにした項目や、Step 0.5 の各レンズで「後で確認」とした事項のうち、追跡すべきものを issue 化する:

```bash
gh issue create \
  --title "<簡潔な follow-up 内容>" \
  --body "<背景・close 条件・関連リリース vX.Y.Z・関連 PR/issue>"
```

issue 化の基準（いずれかに該当したら作成）:
- 未来の日付・外部イベント待ち（docs 公開、価格改定期日など）で今は判定不能
- CI 化・回帰テストなど別 PR に切り出すべき作業
- ローカル未インストールでスキップした ECC 版など、環境が整えば再評価する項目

作成/クローズした issue 番号を Step 9（memory）と Step 11（レポート）に反映する。

> 追跡不要な単発スキップ（テンプレ非該当で恒久的に対象外）は issue 化しない。issue は「いつか再訪する」ものだけに絞る。

---

### 9. Update memory（= memory 面の更新）
...
- Open follow-up issue の一覧（Step 8 の作成/クローズ結果）を反映
- Remove any stale open-PR entries

**反映漏れ防止チェック（必須）**: memory 更新後に `gh release list --limit 1` の最新タグと memory の release history 先頭エントリが一致することを確認する。ズレていたら release history を修正する（過去に v0.77.0 の反映漏れが起きたため恒久チェック）。
...
### 10. Update Obsidian project note（= obsidian 面の更新）

memory ミラー（Step 9）とは別に、**手動運用の project note** を更新する:
